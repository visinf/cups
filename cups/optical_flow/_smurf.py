...
class CorrBlock(nn.Module):
    ...
    def index_pyramid(self, centroids_coords):
        ...
        for i, corr in enumerate(self.corr_pyramid):
            ...
            dx = torch.linspace(int(start), int(stop), int(num), device=centroids_coords.device)
            dy = torch.linspace(int(start), int(stop), int(num), device=centroids_coords.device)
...
class RAFT(nn.Module):
    ...
    def forward(self, image1, image2, num_flow_updates: int = 12):
        ...
        self.corr_block.build_pyramid(fmap1.float(), fmap2.float())
        ...
        # - one part is used to initialize the hidden state of the recurrent units of the update block
        ...
        coords0 = make_coords_grid(batch_size, h // 8, w // 8).to(fmap1.device, image1.dtype)
        coords1 = make_coords_grid(batch_size, h // 8, w // 8).to(fmap1.device, image1.dtype)

        flow_predictions = []
        for _ in range(num_flow_updates):
            coords1 = coords1.detach()  # Don't backpropagate gradients through this branch, see paper
            corr_features = self.corr_block.index_pyramid(centroids_coords=coords1.float()).to(image1.dtype)
