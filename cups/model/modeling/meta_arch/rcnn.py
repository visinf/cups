...
class GeneralizedRCNN(nn.Module):
    ...
    def visualize_training(self, batched_inputs, proposals):
        """A function used to visualize images and proposals. It shows ground truth bounding boxes on the original image
        and up to 20 top-scoring predicted object proposals on the original image. Users can implement different
    ...
    def inference(
        ...
        if detected_instances is None:
            ...
            results = self.roi_heads.forward_with_given_boxes(features, detected_instances)  # type: ignore
    ...
    def preprocess_image(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        assert all(
            [(x["image"].max() <= (1.0 + 0.01)) and (x["image"].min() >= (0.0 - 0.01)) for x in batched_inputs]
        ), "Input images should be in the range [0, 1], found an image not in this range."
